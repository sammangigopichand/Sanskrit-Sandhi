...
def run_benchmark(num_samples=50):
    ...
    # 1. Load Data
    try:
        df = pd.read_csv('sandhikosh_sample.csv')
        # Filter for valid rows
        df = df.dropna(subset=['compound_word', 'word1', 'word2'])
        test_df = df.sample(n=num_samples, random_state=42).reset_index(drop=True)
    except Exception as e:
        print("Error loading dataset:", e)
        return
    
    print(f"Loaded {num_samples} random samples from sandhikosh_sample.csv\n")
    ...
    for idx, row in tqdm(test_df.iterrows(), total=num_samples):
        compound = str(row['compound_word']).strip()
        expected_split = f"{str(row['word1']).strip()} {str(row['word2']).strip()}"
        ...
        if HUGGINGFACE_AVAILABLE and hf_model is not None:
            ...
            # Simple normalizer for evaluation (collapsing multi-spaces)
